...
func checkProcessName(pid int) error {
	name, _ := getProcessExeName(pid)
	selfName, _ := getProcessExeName(os.Getpid())
	if name == selfName {
		return ErrBusy
	} else {
		return nil
