...
func (paths *Path) initPaths(cfg *Path) error {
	*paths = *cfg
	...
	// default for pid file path
	if paths.Pid == "" {
		paths.Pid = paths.Home
	}
...
func (paths *Path) Resolve(fileType FileType, path string) string {
	...
	case Pid:
		return filepath.Join(paths.Pid, path)
	default:
...
func (paths *Path) String() string {
	return fmt.Sprintf("Home path: [%s] Config path: [%s] Data path: [%s] Logs path: [%s] Pid path: [%s]",
		paths.Home, paths.Config, paths.Data, paths.Logs, paths.Pid)
}
