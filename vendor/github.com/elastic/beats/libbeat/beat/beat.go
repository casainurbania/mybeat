...
import (
	...
	bkreloader "github.com/elastic/beats/bkdatalib/reloader"
...
var (
	// add by bkdata
	reload       = flag.Bool("reload", false, "reload config")
...
func (b *Beat) launch(bt Creator) error {
	...
	pidFilePath, err := bkcommon.MakePifFilePath(b.Name, b.Config.Path.Pid)
	if err != nil {
		return err
	}
	// add by bkdata
	if *reload { // reload config
		return bkreloader.ReloadEvent(b.Name, pidFilePath)
	}

	// add by bkdata
	// test mode do not use pidfile
	if !(*common.Test_mode) {
	...
	if !(*common.Test_mode) {
		// config is nil now
		dbFilePath := filepath.Join(b.Config.Path.Data, b.Name+".bkpipe.db")
		err = bkstorage.Init(dbFilePath, nil)
		if err != nil {
			return fmt.Errorf("initializing storage %s error: %v", dbFilePath, err)
		}
		defer bkstorage.Close()
	}
	...
	// start reloader
	if !(*common.Test_mode) {
		reloader := bkreloader.NewReloader(b.Name, beater)
...
func (b *Beat) configure() error {
	...
	// add by bkdata
	// reload not init log, storage again
	if *reload {
		return nil
	}
